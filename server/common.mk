...
#SRCS = nginx.cxx ngx_conf.cxx
SRCS = $(wildcard *.cxx)

#OBJS = nginx.o ngx_conf.o  这么一个一个增加.o太麻烦，下行换一种写法：把字符串中的.cxxxx替换为.o
OBJS = $(SRCS:.cxx=.o)
...
#这里是诸多.d文件被包含进来，每个.d文件里都记录着一个.o文件所依赖哪些.cxx和.h文件。内容诸如 nginx.o: nginx.cxx ngx_func.h
...
#.d文件中的内容可能形如：nginx.o: nginx.cxx ngx_func.h
#%.d:%.cxx
$(DEP_DIR)/%.d:%.cxx
#gcc -MM $^ > $@
#.d文件中的内容形如：nginx.o: nginx.cxx ngx_func.h ../signal/ngx_signal.h，但现在的问题是我们的.o文件已经放到了专门的目录
# 所以我们要正确指明.o文件路径这样，对应的.h,.cxx修改后，make时才能发现，这里需要用到sed文本处理工具和一些正则表达式语法，不必深究
