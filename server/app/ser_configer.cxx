...
bool SerConfiger::Load(const char* pConfFileName) {
    FILE* fp;
...
const char* SerConfiger::GetString(const char* pItemName) {
    for (auto& item : mConfItemList) {
...
int SerConfiger::GetIntDefault(const char* pItemName, int def) {
    for (auto& item : mConfItemList) {
