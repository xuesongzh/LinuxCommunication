...
typedef struct PKG_HEADER {
    unsigned short pkgLen;   //2字节，包头加包体的长度，unsigned short能表示的大小超过30000，可行
    unsigned short msgCode;  //消息码，区别不同的命令
    int crc32;               //crc32校验，数据校验保证数据正确性

} PKG_HEADER, *LPPKG_HEADER;

#pragma pack()  //取消指定对齐，恢复缺省对齐

#define PKG_HEAD_INFO_SIZE (sizeof(PKG_HEADER) + 10)  //接受包体内存大小，一定要比包头的结构所占内存大
