...
int ser_init_signals() {
    ...
    for (sig = signals; sig->signo != 0; ++sig) {
        ...
        if (sig->handler) {
            ...
            //如果你想用sa_sigaction，那么你就需要把sa_flags设置为SA_SIGINFO；
        ...
        if (sigaction(sig->signo, &sa, NULL) == -1) {
            SER_LOG(SER_LOG_EMERG, errno, "sigaction(%s) register failed!", sig->signame);
            return -1;
        } else {
            SER_LOG_STDERR(0, "sigaction(%s) register sucess!", sig->signame);
        }
    }
