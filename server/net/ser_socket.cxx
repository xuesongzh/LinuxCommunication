...
SerSocket::SerSocket() : mListenPortCount(1), mWorkerConnections(1), mEpollFd(-1), mRecyWaiteTime(60) {
...
void SerSocket::ReadConf() {
    ...
    mIfKickTimer = pConfiger->GetIntDefault("WaitTimeEnable", 0);       //是否开启踢人时钟，1：开启   0：不开启
    mWaitTime = pConfiger->GetIntDefault("MaxWaitTime", mWaitTime);     //多少秒检测一次是否 心跳超时，只有当WaitTimeEnable = 1时，本项才有用
    mWaitTime = (mWaitTime > 5) ? mWaitTime : 5;                        //不建议低于5秒钟，因为无需太频繁
    m_ifTimeOutKick = pConfiger->GetIntDefault("Sock_TimeOutKick", 0);  //当时间到达Sock_MaxWaitTime指定的时间时，直接把客户端踢出去，只有当Sock_WaitTimeEnable = 1时，本项才有用

    m_floodAkEnable = pConfiger->GetIntDefault("Sock_FloodAttackKickEnable", 0);    //Flood攻击检测是否开启,1：开启   0：不开启
    m_floodTimeInterval = pConfiger->GetIntDefault("Sock_FloodTimeInterval", 100);  //表示每次收到数据包的时间间隔是100(毫秒)
    m_floodKickCount = pConfiger->GetIntDefault("Sock_FloodKickCounter", 10);
...
bool SerSocket::ser_init_subproc() {
    ...
    //第三个参数=0，表示信号量的初始值，为0时，调用sem_wait()就会卡在那里
    ...
    int err;
    //创建线程，用于发送数据
    ThreadItem* pSendQueueThread = new ThreadItem(this);
    mThreads.push_back(pSendQueueThread);
    ...
    //创建线程用于延迟回收连接池对象
    ThreadItem* pRecyConnThread = new ThreadItem(this);
    mThreads.push_back(pRecyConnThread);
...
bool SerSocket::ser_open_listening_sockets() {
    ...
    for (int i = 0; i < mListenPortCount; ++i) {
        ...
        //设置监听套接字为非阻塞
        if (!ser_set_nonblocking(listenSockFd)) {
        ...
        //监听成功放入列表中
        lpser_listening_t pListenSocketItem = new ser_listening_t();
        ...
        mListenSocketList.emplace_back(pListenSocketItem);
    }
...
int SerSocket::ser_epoll_init() {
    //创建epoll对象，红黑树+双向链表
    mEpollFd = epoll_create(mWorkerConnections);  //参数大于0即可，这里传epoll能处理的最大连接数
    if (-1 == mEpollFd) {
    ...
    //为监听套接字增加一个连接池中的连接
    for (auto& sockListen : mListenSocketList) {
        auto freeConnection = ser_get_free_connection(sockListen->mFd);  //取出一个空闲链接
        if (nullptr == freeConnection) {
            //说明空闲连接没有了，连接池被分配完了
            SER_LOG_STDERR(0, "ser_get_free_connection()取空闲链接失败!");
            exit(2);
        }
        freeConnection->mListening = sockListen;                   //连接对象关联监听套接字对象
        sockListen->mConnection = freeConnection;                  //监听对象关联连接池对象
        freeConnection->mRHandler = &SerSocket::ser_event_accept;  //对监听端口读事件设置处理方法
        //往监听套接字上增加监听事件
        if (-1 == ser_epoll_oper_event(sockListen->mFd, EPOLL_CTL_ADD, EPOLLIN | EPOLLRDHUP, 0, freeConnection)) {
            exit(2);
        }
    }
...
int SerSocket::ser_epoll_oper_event(const int& fd,
                                    const uint32_t& op,
    ...
    ev.data.ptr = (void*)pConnection;  //修复bug,否则会崩溃，因为linux内核对EPOLL_CTL_MOD的实现方式，指针会被置0
    if (epoll_ctl(mEpollFd, op, fd, &ev) == -1) {
...
//返回值 1：正常返回，0：有问题返回。两者出现都需要程序正常继续执行
...
int SerSocket::ser_epoll_process_events(const int& timer) {
    ...
    //阻塞timer这么长时间除非：a)阻塞时间到达 b)阻塞期间收到事件【比如新用户连入】会立刻返回 c)调用时有事件也会立刻返回 d)如果来个信号，比如你用kill -1 pid
    ...
    for (int i = 0; i < events; ++i) {
        ...
        //处理正常事件
        eventType = mEvents[i].events;  //取得事件类型

        if (eventType & (EPOLLERR | EPOLLHUP)) {  //发生了错误或者客户端断连
...
void SerSocket::ser_close_listening_sockets() {
    for (auto& sockListen : mListenSocketList) {
        close(sockListen->mFd);
        SER_LOG(SER_LOG_INFO, 0, "关闭监听端口:%d", sockListen->mFd);
    }
    return;
}
    ...
    while (g_stopEvent == 0) {  //程序不退出
        ...
        if (!pSocket->mMsgSendQueue.empty()) {  //发送消息队列不为空需要发送消息
            ...
            while (iter != iterEnd) {
                ...
                if (pConnection->mThrowSendCount > 0) {  //是否靠epoll驱动来发消息
                ...
                pConnection->mSendLength = htons(pPkgHeader->pkgLen);
                ...
                sendSize = pSocket->ser_send_pkg(pConnection->mSockFd, pConnection->mSendLocation, pConnection->mSendLength);
                if (sendSize > 0) {
                    if (sendSize == pConnection->mSendLength) {  //一次性成功发送完了数据
                        pMemory->FreeMemory(pConnection->mSendPkgData);
                        pConnection->mThrowSendCount = 0;
                        SER_LOG(SER_LOG_DEBUG, 0, "数据发送完毕");
                    } else {
                        //数据没有发完，肯定是因为缓冲区满了，这时需要为epoll对象添加写事件
                        pConnection->mSendLocation = pConnection->mSendLocation + sendSize;  //更新发送数据的位置
                        pConnection->mSendLength = pConnection->mSendLength - sendSize;      //更新发送数据大小
                        ++pConnection->mThrowSendCount;                                      //标记需要通过epoll事件驱动来发送数据
                        if (pSocket->ser_epoll_oper_event(pConnection->mSockFd,
                                                          EPOLL_CTL_MOD,
                                                          EPOLLOUT,
                                                          0,
                                                          pConnection) == -1) {
                        ...
                        SER_LOG_STDERR(0, "发送缓冲区满了，应该发送：%d,实际发送：%d。此时需要靠epoll驱动发数据!", pConnection->mSendLength, sendSize);
                    }
                    continue;
                } else if (sendSize == 0) {
